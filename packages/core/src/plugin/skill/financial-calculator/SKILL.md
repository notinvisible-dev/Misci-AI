...
1. Ask what I'm trying to figure out. Common scenarios include:
...
2. Gather the inputs I need. Pull from memory first — only ask for what's new or has likely changed. Group related inputs together. For each input, explain briefly why it matters so the user understands what drives the result.

3. If I'm missing a number and it's reasonable to estimate, offer a default with an explanation: "I'll assume [X] — that's typical for [reason]. Want to adjust?" Never silently assume — always surface assumptions.
...
6. Ask if they want to tweak any inputs or run additional scenarios. Make it easy to adjust one variable at a time and see the impact.
